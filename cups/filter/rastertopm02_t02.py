...
def select_justification(file: BufferedWriter, justification: int = 1):
    _ = file.write(ESC + b'a') # select justification
    _ = file.write(justification.to_bytes(1, 'little'))
    return

def print_header(file: BufferedWriter):
    printer_init(file)
...
def print_and_feed(file: BufferedWriter, lines: int = 1):
    _ = file.write(ESC + b'd') # print and feed
    _ = file.write(lines.to_bytes(1, 'little'))
